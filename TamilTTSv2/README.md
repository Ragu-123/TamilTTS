...
**Branch note:** `main` holds the legacy V1 stack (RAD-TTS learned aligner + diffusion). This branch, `feature/tamiltts-v2`, is the current V2 stack — the repo root now contains ONLY `TamilTTSv2/` (the duplicate V1 copy that used to sit at root was removed; it remains intact and untouched on `main`).
